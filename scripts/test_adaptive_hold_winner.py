"""
Testar o melhor resultado do Optuna vs Buy & Hold puro
"""

import sys
from pathlib import Path
import pandas as pd

project_root = Path(__file__).parent.parent
sys.path.insert(0, str(project_root))

from engines.data_engine import DataEngine
from engines.backtest_engine import BacktestEngine
from strategies.btc_adaptive_hold import BTCAdaptiveHold
from rich.console import Console
from rich.table import Table

console = Console()

# Melhores parâmetros do Trial #195
best_params = {
    'rsi_period': 16,
    'rsi_exit_threshold': 18,
    'macd_fast': 8,
    'macd_slow': 30,
    'macd_signal': 8,
    'macd_exit_threshold': -27.600280391453623,
    'volume_period': 27,
    'volume_panic_multiplier': 2.577083217444588,
    'lookback_dd': 12,
    'drawdown_exit_threshold': -13.867214706291536,
    'bb_period': 22,
    'bb_std': 1.5654064982171376,
    'bb_exit_lower_mult': 0.36762843511517485,
    'atr_period': 14,
    'atr_exit_multiplier': 2.8616783165204462,
    'momentum_period': 6,
    'momentum_exit_threshold': -0.05088268915113008,
    'ema_fast': 22,
    'ema_slow': 65,
    'use_ema_cross_exit': False,
    'min_exit_signals': 6,
    'rsi_entry_threshold': 31,
    'macd_entry_threshold': 11.062550340233276,
    'macd_entry_crossover': True,
    'volume_entry_max': 2.1179753122661986,
    'lookback_recovery': 11,
    'recovery_threshold': 0.1104076267326576,
    'bb_entry_position': 0.21272798467460416,
    'atr_entry_multiplier': 2.285265551788009,
    'momentum_entry_threshold': 0.00844127442543361,
    'use_ema_cross_entry': False,
    'check_higher_lows': True,
    'higher_lows_period': 5,
    'min_entry_signals': 1,
    'position_size': 0.9859798209269933,
    'hold_period_after_exit': 7,
}

# Load data
data_engine = DataEngine(use_cache=True, auto_indicators=True)
df = data_engine.load_prices(symbol='BTC-USD', start='2020-01-01', end='2025-11-24')

# Calcular buy & hold
first_price = df.iloc[0]['close']
last_price = df.iloc[-1]['close']
buyhold_return = ((last_price - first_price) / first_price) * 100

console.print(f"\n[bold cyan]BTC ADAPTIVE HOLD - VERIFICAÇÃO DO VENCEDOR[/bold cyan]")
console.print(f"\nPeriodo: 2020-01-01 → 2025-11-24")
console.print(f"Preço inicial: ${first_price:,.2f}")
console.print(f"Preço final: ${last_price:,.2f}")
console.print(f"\n[yellow]Buy & Hold Puro: +{buyhold_return:.2f}%[/yellow]\n")

# Run backtest
backtest_engine = BacktestEngine(initial_cash=100000, commission=0.001)
results = backtest_engine.run_backtest(
    strategy_cls=BTCAdaptiveHold,
    data_df=df,
    symbol='BTC-USD',
    **best_params
)

if results:
    return_pct = results.get('return_pct', 0)
    analyzers = results.get('analyzers', {})
    sharpe = analyzers.get('sharpe', 0) or 0
    max_dd = abs(analyzers.get('max_drawdown', 0))
    total_trades = analyzers.get('total_trades', 0)
    won = analyzers.get('won_trades', 0)
    lost = analyzers.get('lost_trades', 0)
    win_rate = analyzers.get('win_rate', 0)
    
    alpha = return_pct - buyhold_return
    
    # Tabela de comparação
    table = Table(title="\n📊 RESULTADO FINAL", show_header=True, header_style="bold magenta")
    table.add_column("Métrica", style="cyan", justify="left")
    table.add_column("Buy & Hold", justify="right", style="yellow")
    table.add_column("Adaptive Hold", justify="right", style="green")
    table.add_column("Diferença", justify="right", style="red")
    
    table.add_row(
        "Retorno Total",
        f"+{buyhold_return:.2f}%",
        f"+{return_pct:.2f}%",
        f"{alpha:+.2f}%"
    )
    
    table.add_row(
        "Capital Final",
        f"${100000 * (1 + buyhold_return/100):,.2f}",
        f"${100000 * (1 + return_pct/100):,.2f}",
        f"${100000 * (alpha/100):+,.2f}"
    )
    
    table.add_row(
        "Max Drawdown",
        f"{51.9:.1f}%",  # B&H histórico BTC
        f"{max_dd:.1f}%",
        f"{max_dd - 51.9:+.1f}%"
    )
    
    table.add_row(
        "Sharpe Ratio",
        "~0.65",  # Estimativa B&H
        f"{sharpe:.3f}",
        f"{sharpe - 0.65:+.3f}"
    )
    
    table.add_row(
        "Total Trades",
        "1 (compra inicial)",
        f"{total_trades}",
        f"{total_trades - 1:+d}"
    )
    
    table.add_row(
        "Win Rate",
        "100%",
        f"{win_rate:.1f}%",
        f"{win_rate - 100:+.1f}%"
    )
    
    console.print(table)
    
    # Conclusão
    if alpha > 0:
        console.print(f"\n[bold green]✅ GANHOU DO BUY & HOLD POR {alpha:.2f}%![/bold green]")
        console.print(f"[green]Alpha: +{alpha:.2f}% ({alpha/buyhold_return*100:.1f}% melhor)[/green]")
    else:
        console.print(f"\n[bold red]❌ PERDEU PARA BUY & HOLD POR {abs(alpha):.2f}%[/bold red]")
        console.print(f"[red]Alpha: {alpha:.2f}% ({abs(alpha)/buyhold_return*100:.1f}% pior)[/red]")
    
    # Análise detalhada
    console.print("\n[bold]📊 ANÁLISE DETALHADA[/bold]")
    console.print(f"\n[yellow]Total de trades: {total_trades}[/yellow]")
    
    if total_trades == 1:
        console.print("[green]→ Estratégia é essencialmente BUY & HOLD![/green]")
        console.print("[green]→ Não saiu em nenhum momento (exit signals nunca ativaram)[/green]")
        console.print("[green]→ Alpha vem do timing de entrada ligeiramente melhor[/green]")
    elif total_trades <= 3:
        console.print(f"[yellow]→ Muito poucos trades ({total_trades})[/yellow]")
        console.print("[yellow]→ Estratégia quase igual a buy & hold[/yellow]")
    else:
        console.print(f"[blue]→ {total_trades} trades executados[/blue]")
        console.print(f"[blue]→ Win rate: {win_rate:.1f}%[/blue]")
        console.print(f"[blue]→ Won: {won}, Lost: {lost}[/blue]")
    
    console.print(f"\n[yellow]Drawdown: {max_dd:.1f}%[/yellow]")
    if max_dd < 52:
        console.print("[green]→ DD menor que buy & hold puro (51.9%)[/green]")
    elif max_dd < 60:
        console.print("[yellow]→ DD similar ao buy & hold[/yellow]")
    else:
        console.print("[red]→ DD maior que buy & hold puro[/red]")
    
    console.print(f"\n[yellow]Sharpe: {sharpe:.3f}[/yellow]")
    if sharpe > 0.7:
        console.print("[green]→ Risk-adjusted return bom[/green]")
    else:
        console.print("[yellow]→ Risk-adjusted return mediano[/yellow]")
