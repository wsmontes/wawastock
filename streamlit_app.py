...
# ========== TOP NAVIGATION BAR ==========
# Compact header with tabs-style navigation
col_title, col_nav = st.columns([2, 1])

with col_title:
    st.markdown("### 📊 WawaStock Backtest")

with col_nav:
    page = st.segmented_control(
        "Navigation",
        options=["Backtest", "Analysis"],
        default="Backtest" if st.session_state.get('page', 'backtest') == 'backtest' else "Analysis",
...
Commission: {commission}%
    ...
    # ========== ANALYSIS PAGE ==========
    from streamlit_pages import analysis_page
    analysis_page.render(bridge)
